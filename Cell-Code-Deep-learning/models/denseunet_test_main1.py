...
def process_images(image1_path, image2_path, save_path1, save_path2):
    """
    处理两张图片并保存结果
    ...
    Args:
        ...
        save_path1: 第一张图片处理结果的保存路径
        save_path2: 第二张图片处理结果的保存路径
    """
    import sys
    import os
    model_weight_path = './Cell-Code-Deep-learning/weights/denseunet/denseunet_CNNedge_weights.h5'
    ...
    if not os.path.exists(model_weight_path):
        ...
        if not os.path.exists(model_weight_path):
            # 尝试在项目根目录查找
            model_weight_path = os.path.join(os.path.dirname(__file__), '..', '..', 'weights', 'denseunet', 'denseunet_CNNedge_weights.h5')
            if not os.path.exists(model_weight_path):
                print(f"错误: 找不到模型权重文件 {model_weight_path}")
                print("请确保模型权重文件存在于指定路径")
                return False
    ...
    # 处理第一张图片
    print(f"处理图片: {image1_path}")
    if not os.path.exists(image1_path):
        print(f"错误: 找不到图片文件 {image1_path}")
        return False
        
    init_image1 = imageio.imread(image1_path)[:, :, 0]
    init_image1 = init_image1[np.newaxis, :, :, np.newaxis]  # Add axis C.
    init_image1 = init_image1.astype(np.float32) / 255  # Normalize
    edge_image1 = model.predict_on_batch(init_image1)
    edge_image1 = edge_image1[:, :, 1]
    edge_image1 = np.reshape(edge_image1, (1, init_image1.shape[1], init_image1.shape[2]))[0]
    plt.imsave(save_path1, edge_image1, cmap='gray')
    
    # 处理第二张图片
    print(f"处理图片: {image2_path}")
    if not os.path.exists(image2_path):
        print(f"错误: 找不到图片文件 {image2_path}")
        return False
        
    init_image2 = imageio.imread(image2_path)[:, :, 0]
    init_image2 = init_image2[np.newaxis, :, :, np.newaxis]  # Add axis C.
    init_image2 = init_image2.astype(np.float32) / 255  # Normalize
    edge_image2 = model.predict_on_batch(init_image2)
    edge_image2 = edge_image2[:, :, 1]
    edge_image2 = np.reshape(edge_image2, (1, init_image2.shape[1], init_image2.shape[2]))[0]
    plt.imsave(save_path2, edge_image2, cmap='gray')
    
    print(f"处理完成，结果已保存到: {save_path1} 和 {save_path2}")
    return True

# 示例调用
if __name__ == "__main__":
    # 使用示例
    image1_path = '../0525/img_fli.jpg'
    image2_path = '../0525/img_new.jpg'
    save_path1 = '../0525/results/img_fli_pre.jpg'
    save_path2 = '../0525/results/img_new_pre.jpg'
    ...
    if len(sys.argv) > 4:
        ...
        save_path1 = sys.argv[3]
        save_path2 = sys.argv[4]
    
    # 确保保存目录存在
    save_dir = os.path.dirname(save_path1)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    
    process_images(image1_path, image2_path, save_path1, save_path2)
